...
d3.csv(url).then(data => { // Get Data
  ...
  var numDep = document.querySelector("input[id='slNumBars']").value;
...
d3.csv(url).then(data => { // Get Data
  ...
  // console.log("iwidth: " + iwidth + "    " + "iheight: " + iheight);x
  const munLength = d3.set(data.map(d => d.MUNICIPIO)).values();

  d3.select("#slNumBars")
    .on('change', onchange2);

  console.log("Length", munLength.length);

  function onchange2() { // change the values

    console.log("Length:  ", munLength.length);
    d3.select("#target4").remove(); // clear the bars

    d3.select('#targetD')
      .append('div')
      .attr("id", "target4");

    var numDep = document.querySelector("input[id='slNumBars']").value; //get the slide value

    console.log("Change");
    console.log("Value: " + numDep);
    // }; // Close here
...
d3.csv(url).then(data => { // Get Data
  ...
  d3.select("#target")
    .append("h6")
    .text("Seleccione el departamento que desea visualizar");
